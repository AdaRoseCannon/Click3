define([], function () {
    ...
    return function (listenTarget) {
        ...
        var getEventLocation = function (e) {
            var newPos;
            if (e.clientX) newPos = {x: e.clientX, y: e.clientY};
        ...
        var mousemove = function (e) {
            ...
            if (newPos === undefined) return;
            oldPos = newPos;
            var event = new window.CustomEvent('pi-move', {'detail': newPos});
            el.dispatchEvent(event);

            var distance = Math.sqrt(Math.pow(newPos.x - interactionStartLocation.x, 2) + Math.pow(newPos.y - interactionStartLocation.y, 2));
            var displacement = {x: newPos.x - interactionStartLocation.x, y: newPos.y - interactionStartLocation.y};
            if(distance >= dragTriggerRadius) {
                var eventDrag = new window.CustomEvent('pi-drag', {'detail': {current: newPos, distance: distance, displacement: displacement}});
                if (dragging) {
                ...
                if (!dragging) {
                    dragTimeout = Date.now();
                    dragging = true;
                    var event2 = new window.CustomEvent('pi-dragStart', {'detail': newPos});
        ...
        var mousedown = function (e) {
            ...
            interactionStartLocation =  {x: newPos.x, y: newPos.y};
        ...
        var mouseup = function (e, noclick) {
            ...
            if (dragging === false && !noclick && length <= 500 || distance <= dragTriggerRadius) {
                var event = new window.CustomEvent('pi-click', {'detail': newPos});
                el.dispatchEvent(event);
            }
            if ( dragging === true) {
                dragging = false;
                var eventDragEnd = new window.CustomEvent('pi-dragEnd', {'detail': newPos});
